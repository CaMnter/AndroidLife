...
import com.camnter.patch.ref.ClassReferenceListBuilder
import com.camnter.patch.ref.PatchRefScan
import com.camnter.patch.ref.Path
...
class NuwaProcessor {
        ...
        if (jarFile) {
            ...
            if (RocooFixPlugin.rocooConfig.scanref) {
                ...
                while (enumerationForScanref.hasMoreElements()) {
                    ...
                    PatchRefScan.addClassIfRefPatchClass(jarEntry, entryName, allrefs, patchClasss,
                            file, patchDir);
                    mainListBuilder.clearAllForReuse();
                }
                //
            ...
            public MethodVisitor visitMethod(int access, String name, String desc,
                ...
                mv = new MethodVisitor(Opcodes.ASM4, mv) {
                    ...
                    void visitInsn(int opcode) {
                        if (("<init>".equals(name) || "<clinit>".equals(name)) && opcode ==
                                Opcodes.RETURN &&
                                !hasHackSuccess) {
                            //  第一次尝试hack
    ...
    private static byte[] addCinitAndHack(ClassReader cr, ClassWriter cw) {
        MethodVisitor constructor = cw.visitMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC,
                "<clinit>", "()V", null, null);
    ...
    public static boolean shouldProcessPreDexJar(String path) {
        return path.endsWith("classes.jar") && !path.contains("com.android.support") &&
    ...
    static boolean shouldProcessClassInJar(String entryName, HashSet<String> includePackage,
        ...
        return entryName.endsWith(".class") && !entryName.startsWith("com/dodola/rocoofix/") &&
